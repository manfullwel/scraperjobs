...
class JobScraper:
    def __init__(self):
        ...
        self.job_sites = {
            ...
            'Vagas.com.br': {
                'base_url': 'https://www.vagas.com.br/vagas-de-{}?ordenar_por=mais_recentes',
                'selectors': {
                    'job_cards': '.vaga',
                    'title': 'a.link-detalhes-vaga',
                    'company': '.emprVaga',
                    'location': '.local',
                    'description': '.detalhes',
                    'posted_date': '.data-publicacao',
                    'job_type': '.nivelVaga',
                    'salary': '.infoVaga'
                }
            },
            'Gupy': {
                'base_url': 'https://portal.gupy.io/job-search?searchTerm={}',
                'selectors': {
                    ...
                    'posted_date': '[data-testid="job-card-date"]',
                    'job_type': '[data-testid="job-card-type"]',
                    'salary': '[data-testid="job-card-salary"]'
                }
            }
        }
    ...
    def _scrape_site(self, page, site_name, site_info, keywords, location):
        ...
        try:
            ...
            elif site_name == 'Vagas.com.br':
                url = site_info['base_url'].format(quote_plus(keywords))
            elif site_name == 'Gupy':
                url = site_info['base_url'].format(quote_plus(keywords))
...
class JobSearchApp:
    ...
    def setup_ui(self):
        ...
        options_frame.grid(row=1, column=0, columnspan=4, pady=5)
    ...
    def start_search(self):
        ...
        if not keywords:
            messagebox.showwarning("Aviso", "Digite palavras-chave para buscar")
            return
        ...
        threading.Thread(target=self.search_jobs, args=(keywords, location), daemon=True).start()
    ...
    def search_jobs(self, keywords, location):
        try:
            ...
            # Scrape jobs
            all_jobs = self.scraper.scrape_jobs(keywords, location)
